...
// Componente de Hábitos Semanales: Gráfico circular unificado + input con "+" para añadir strings y objetos de tareas
const WeeklyHabits = ({ habits, records, toggleRecord, removeHabit, visibleDays, weeklyNotes, updateWeeklyNote }) => {
    ...
    // Generar un ID único para la semana actual basado en el primer día
    const startDayDate = visibleDays[0];
    const weekKey = `week-${startDayDate.getFullYear()}-${startDayDate.getMonth()+1}-${startDayDate.getDate()}`;
    ...
    // Progreso total para el gráfico: hábitos semanales + ítems personalizados de la ventana actual
    let totalExpected = 0;
    let totalDone = 0;
    ...
    const handleAddItem = () => {
        ...
        if (!text) return;
        updateWeeklyNote(weekKey, [...items, { text, done: false }]);
        setWeekInput('');
    };

    const handleToggleItem = (idx) => {
        updateWeeklyNote(weekKey, items.map((item, i) =>
            i === idx ? { ...item, done: !item.done } : item
        ));
    };

    const handleDeleteItem = (idx) => {
        updateWeeklyNote(weekKey, items.filter((_, i) => i !== idx));
    };

    const startDay = visibleDays[0].getDate();
    const endDay = visibleDays[visibleDays.length - 1].getDate();
    ...
    return (
        <div className="mt-8 bg-dark-card border-t border-border-subtle p-6 flex flex-col xl:flex-row gap-8">
            ...
            {/* Columnas por semana (ahora 1 sola basada en visibleDays) */}
            <div className="flex-1 overflow-x-auto">
                <div className="flex min-w-max gap-4 w-full">
                    <div className="w-full xl:w-1/2 rounded-lg overflow-hidden border border-border-subtle flex flex-col shadow-sm">
                        {/* Cabecera */}
                        <div className="bg-dark-main p-3 text-center border-b border-border-subtle">
                            <h4 className="text-[11px] font-black text-text-primary uppercase tracking-widest">ESTA SEMANA</h4>
                        ...
                        <div className="p-3 space-y-1 flex-1">
                            ...
                            {weeklyHabits.map(habit => {
                                ...
                                return (
                                    <div key={habit.id} className="flex items-center gap-2 p-2 hover:bg-dark-main rounded transition-colors group">
                                        <div
                                            className={`w-5 h-5 rounded-full border-2 transition-all flex items-center justify-center shrink-0 cursor-pointer ${isChecked ? 'bg-blue-500 border-blue-500' : 'bg-dark-card border-border-subtle'}`}
                                            onClick={() => toggleRecord(weekKey, habit.id)}
                                        >
                                            {isChecked && (
                                                <svg className="w-3 h-3 text-dark-main" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            {items.map((item, idx) => (
                                <div key={idx} className="flex items-center gap-2 p-2 hover:bg-dark-main rounded transition-colors group">
                                    <div
                                        className={`w-5 h-5 rounded-full border-2 transition-all flex items-center justify-center shrink-0 cursor-pointer ${item.done ? 'bg-blue-500 border-blue-500' : 'bg-dark-card border-border-subtle'}`}
                        ...
                        <div className="px-3 pb-3 mt-2">
                            <form className="flex gap-2" onSubmit={(e) => { e.preventDefault(); handleAddItem(); }}>
                                <input
                                    type="text"
                                    className="flex-1 text-xs p-1.5 rounded border border-border-subtle focus:outline-none focus:border-blue-400 bg-dark-card text-text-primary"
                                    placeholder="Añadir ítem..."
                                    value={weekInput}
                                    onChange={e => setWeekInput(e.target.value)}
                                />
                                <button type="submit" className="bg-blue-500 hover:bg-blue-600 text-white rounded w-7 h-7 flex items-center justify-center shrink-0 transition-colors">
                                    <Plus size={14} />
                                </button>
                            </form>
                        </div>
