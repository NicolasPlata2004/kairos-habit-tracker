...
const WeeklyReview = ({ isOpen, onClose, initialMeasurements, onSave }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/80 backdrop-blur-sm">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl h-[90vh] md:h-[80vh] flex flex-col md:flex-row overflow-hidden border border-gray-200 animate-in zoom-in-95 duration-200">
                
                {/* LADO IZQUIERDO: Formulario de Calibración */}
                <div className="w-full md:w-1/3 p-6 md:p-8 bg-gray-50 flex flex-col overflow-y-auto border-r border-gray-200">
                    <div className="flex justify-between items-center mb-6">
                        <div>
                            <h2 className="text-2xl font-black text-gray-800 flex items-center gap-2">
                                <Ruler size={24} className="text-blue-500" /> Calibración SOMA
                            </h2>
                            <p className="text-xs text-gray-500 font-mono mt-1">REVISIÓN SEMANAL PROFUNDA</p>
                        </div>
                        <button onClick={onClose} className="p-2 bg-gray-200 hover:bg-red-100 hover:text-red-500 rounded-full transition-colors md:hidden">
                            <X size={20} />
                        </button>
                    </div>
                    ...
                    <div className="space-y-4 flex-1">
                        <div>
                            <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Género de Ingeniería</label>
                            <select 
                                name="gender" 
                                value={measurements.gender} 
                                onChange={handleChange}
                                className="w-full p-2.5 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-400 focus:border-blue-400 text-sm"
                            >
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="height" value={measurements.height} onChange={handleChange} className="w-full p-2.5 bg-white border border-gray-300 rounded-xl text-sm" />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Cuello (cm)</label>
                                <input type="number" name="neck" value={measurements.neck} onChange={handleChange} className="w-full p-2.5 bg-white border border-gray-300 rounded-xl text-sm" />
                            </div>
                        </div>

                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Cintura (cm)</label>
                                <input type="number" name="waist" value={measurements.waist} onChange={handleChange} className="w-full p-2.5 bg-white border border-gray-300 rounded-xl text-sm" />
                            </div>
                            {measurements.gender === 'female' && (
                                <div>
                                    <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Cadera (cm)</label>
                                    <input type="number" name="hip" value={measurements.hip} onChange={handleChange} className="w-full p-2.5 bg-white border border-gray-300 rounded-xl text-sm" />
                                </div>
                            )}
                        </div>
                    ...
                    <button 
                        onClick={handleSave}
                        className="mt-6 w-full flex items-center justify-center gap-2 bg-gray-900 hover:bg-black text-white p-4 rounded-xl font-bold transition-transform active:scale-95 shadow-lg"
                    >
